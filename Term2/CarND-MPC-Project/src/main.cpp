...
int main() {
  ...
  MPC mpc;
  MPC_Configuration mpc_conf = MPC_Configuration(15);
    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          #pragma Define coeffs
            // Transform ptsx, ptsy to coords
          ...
          #pragma Define state
            ...
            double Lf = mpc_conf.Lf;// 2.67;
            double latency = mpc_conf.latency; //0.1;   
             
            double state_x = v * latency;
            double state_y = 0;
            double state_psi = - v * delta / Lf * latency;
            double state_v = v + throttle * latency;
            double state_cte = cte + v * CppAD::sin(epsi) * latency;
            double state_epsi = epsi + state_psi;
          ...
          #pragma Calculate steering angle and throttle using MPC optimizer to minimize cost
            // TODO: Calculate steering angle and throttle using MPC.  Both are in between [-1, 1].            
            // NOTE: Remember to divide by deg2rad(25) before you send the steering value back.
            // Otherwise the values will be in between [-deg2rad(25), deg2rad(25] instead of [-1, 1].

            auto result = mpc.Solve(state, coeffs, mpc_conf);       
